...
BOT_TOKEN = environ['8267886191:AAGt6by6GtgvGoE-p2VvB5G0TV0ixiBs4V8']
...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '-1002577824824').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '1346360451').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '1346360451').split()]
...
# MongoDB information
DATABASE_URL = environ.get('DATABASE_URL', "mongodb+srv://safin:x0EwcqRWeyafnQIo@cluster0.1qpfh4t.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
DATABASE_NAME = environ.get('DATABASE_NAME', "Safin")
...
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'moviestoreupdates')
...
FILE_STORE_CHANNEL = [int(ch) for ch in (environ.get('FILE_STORE_CHANNEL', '-1002577824824')).split()]
